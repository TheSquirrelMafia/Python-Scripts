...
def extract_metadata(file_path):
    try:
        # Extract ISO, DRE, Model, and ImageTone
        result = subprocess.run(
            ["C:\\exiftool.exe", "-ISO", "-DynamicRangeExpansion", "-Model", "-ImageTone", "-S", file_path],
            capture_output=True,
            text=True
        )
        lines = result.stdout.strip().splitlines()
        iso, dre, model, tone = None, None, None, None
        for line in lines:
        ...
        for file in files:
            if file.lower().endswith(('.nef', '.cr2', '.arw', '.pef', '.dng')):
                ...
                if iso is not None and model is not None:
                    ...
                    destination_path = os.path.join(iso_dre_dir, new_file_name)
...
if __name__ == "__main__":
    ...
    print("Raw files sorted into Model/ISO/DRE directories and renamed with ImageTone.")
