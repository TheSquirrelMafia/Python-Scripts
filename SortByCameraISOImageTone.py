...
def extract_metadata(file_path):
    try:
        # Extract ISO, Model, and ImageTone
        result = subprocess.run(
            ["C:\\exiftool.exe", "-ISO", "-Model", "-ImageTone", "-S", file_path],
            capture_output=True,
            text=True
        )
        lines = result.stdout.strip().splitlines()
        iso, model, tone = None, None, None
        for line in lines:
...
if __name__ == "__main__":
    ...
    print("Raw files sorted into Model/ISO directories and renamed with ImageTone.")
